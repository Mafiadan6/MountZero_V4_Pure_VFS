#!/system/bin/sh
# MountZero v4 - SUSFS Hiding Engine (BRENE Integration)
...
PERSISTENT_DIR="/data/adb/mountzero_v4"
CONFIG_FILE="$PERSISTENT_DIR/config.toml"
SUSFS_BIN="/data/adb/ksu/bin/mz4"
...
# Fallback to ksu_mz4 if mz4 not available
if [ ! -x "$SUSFS_BIN" ]; then
    SUSFS_BIN="/data/adb/ksu/bin/mz4"
fi

log() {
    echo "mountzero_v4-hiding: $*" > /dev/kmsg 2>/dev/null
...
apply_sus_paths() {
    ...
    # KSU/APatch paths
    mz4_add_path "/data/adb/ksu"
    mz4_add_path "/data/adb/ap"
    mz4_add_path "/data/adb/modules"
    mz4_add_path "/data/adb/modules_update"

    # Magisk paths
    mz4_add_path "/sbin/.magisk"
    mz4_add_path "/data/cache/magisk.log"
    mz4_add_path "/cache/recovery"

    # Common detection paths
    mz4_add_path "/data/local/tmp"
    mz4_add_path "/data/adb/mountzero_v4"
...
apply_sus_maps() {
    ...
    mz4_add_map "/data/adb/modules/rezygisk/zygisk"
    mz4_add_map "/data/adb/modules/rezygisk/lib64/libzygisk.so"
    mz4_add_map "/data/adb/modules/rezygisk/lib/libzygisk.so"
    mz4_add_map "/data/adb/modules/zygisk_lsposed"
    mz4_add_map "/data/adb/modules/zygisk-detach/zygisk"
...
apply_uname_spoofing() {
    local ENABLED_FILE="/data/adb/mountzero_v4/uname_spoof_enabled"
    ...
    if [ -f "/data/adb/mountzero_v4/uname_release" ]; then
        kernel_release=$(cat /data/adb/mountzero_v4/uname_release 2>/dev/null)
    fi
    if [ -f "/data/adb/mountzero_v4/uname_version" ]; then
        kernel_version=$(cat /data/adb/mountzero_v4/uname_version 2>/dev/null)
    fi
...
apply_lsposed_hiding() {
    ...
    # Hide dex2oat paths
    mz4_add_map "/system/apex/com.android.art/bin/dex2oat"
    mz4_add_map "/system/apex/com.android.art/bin/dex2oat32"
    mz4_add_map "/system/apex/com.android.art/bin/dex2oat64"
    mz4_add_map "/apex/com.android.art/bin/dex2oat"
    mz4_add_map "/apex/com.android.art/bin/dex2oat32"
    mz4_add_map "/apex/com.android.art/bin/dex2oat64"
...
apply_ext4_loop_hiding() {
    ...
    for device in $(ls -Ld /proc/fs/jbd2/loop*8 2>/dev/null | sed 's|/proc/fs/jbd2/||; s|-8||'); do
        mz4_add_path "/proc/fs/jbd2/${device}-8"
        mz4_add_path "/proc/fs/ext4/${device}"
    done
...
apply_sdcard_android_data_hiding() {
    ...
    for pkg in $(pm list packages -3 2>/dev/null | cut -d':' -f2); do
        if [ -e "/sdcard/Android/data/${pkg}" ]; then
            mz4_add_path "/sdcard/Android/data/${pkg}"
        fi
    done
...
apply_recovery_paths_hiding() {
    ...
    mz4_add_path "/system/addon.d"
    mz4_add_path "/vendor/bin/install-recovery.sh"
    mz4_add_path "/system/bin/install-recovery.sh"
    mz4_add_path "/sdcard/TWRP"
    mz4_add_path "/sdcard/OpenRecovery"
    mz4_add_path "/sdcard/TWRP_backup"
...
/data/adb/ksu/bin/mz4 add_sus_path_loop "$FILE" 2>/dev/null
HANDLER
