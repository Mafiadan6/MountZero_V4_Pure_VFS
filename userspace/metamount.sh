#!/system/bin/sh
# MountZero v4 - KSU/APatch Metamodule Mount Hook
...
LOCKFILE="/dev/mountzero_v4_metamount_lock"
...
cleanup_stale_mounts() {
    ...
    # Unmount any leftover mountzero_v4 overlay mounts
    grep "mountzero_v4_work\|mountzero_v4_upper" /proc/mounts 2>/dev/null | while read -r line; do
        local mp
...
mount_partition_overlay() {
    ...
    # Mount overlay
    mount -t overlay "mountzero_v4_${modid}_${partition}" \
...
if [ -n "$ABI" ] && [ -x "$MZCTL" ]; then
    ...
    # Step 3: Enable MountZero v4 engine
    echo "$LOG: enabling MountZero v4" > /dev/kmsg 2>/dev/null
    "$MZCTL" enable 2>/dev/null
