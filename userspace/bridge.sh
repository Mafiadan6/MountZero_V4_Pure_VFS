...
bridge_write_susfs() {
    ...
    local path_hide
    path_hide=$("$MZ_DATA/config.sh" get mz4 path_hide true 2>/dev/null)
    if [ "$path_hide" = "true" ]; then
        ...
        while IFS= read -r path; do
            ...
            [ -z "$path" ] && continue
            $mzctl mz4 add-path "$path" 2>/dev/null
            count=$((count + 1))
    ...
    local maps_hide
    maps_hide=$("$MZ_DATA/config.sh" get mz4 maps_hide true 2>/dev/null)
    if [ "$maps_hide" = "true" ]; then
        while IFS= read -r map; do
            [ -z "$map" ] && continue
            map=$(echo "$map" | sed 's/^[[:space:]]*//' | sed 's/[[:space:]]*$//')
            [ -z "$map" ] && continue
            $mzctl mz4 add-map "$map" 2>/dev/null
            count=$((count + 1))
    ...
    local kstat
    kstat=$("$MZ_DATA/config.sh" get mz4 kstat true 2>/dev/null)
    if [ "$kstat" = "true" ]; then
        $mzctl mz4 kstat enable 2>/dev/null
        count=$((count + 1))
    fi

    # Apply SUSFS log
    local susfs_log
    susfs_log=$("$MZ_DATA/config.sh" get mz4 susfs_log false 2>/dev/null)
    if [ "$susfs_log" = "true" ]; then
        $mzctl mz4 log enable 2>/dev/null
        count=$((count + 1))
    fi

    # Apply AVC log spoofing
    local avc_log
    avc_log=$("$MZ_DATA/config.sh" get mz4 avcLogSpoofing false 2>/dev/null)
    if [ "$avc_log" = "true" ]; then
        $mzctl mz4 avc enable 2>/dev/null
        count=$((count + 1))
    fi

    # Apply uname spoofing
    local uname_release uname_version
    uname_release=$("$MZ_DATA/config.sh" get mz4 uname_release "" 2>/dev/null)
    uname_version=$("$MZ_DATA/config.sh" get mz4 uname_version "" 2>/dev/null)
    if [ -n "$uname_release" ] && [ -n "$uname_version" ]; then
        $mzctl mz4 set-uname "$uname_release" "$uname_version" 2>/dev/null
        count=$((count + 1))
    fi
    ...
    if [ "$spoof_cmdline" = "true" ]; then
        ...
        if [ -f "$cmdline_file" ]; then
            $mzctl mz4 set-cmdline "$cmdline_file" 2>/dev/null
            count=$((count + 1))
        fi
    fi
    ...
    if [ "$hide_mounts" = "true" ]; then
        /data/adb/ksu/bin/mz4 hide_sus_mnts_for_non_su_procs 1 2>/dev/null
        count=$((count + 1))
    fi
