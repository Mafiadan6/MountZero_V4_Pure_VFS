#!/system/bin/sh
# MountZero v4 - ADB Root via Axon Injection
...
log() {
    echo "mountzero_v4-adb: $*" > /dev/kmsg 2>/dev/null
}
...
patch_linker_config() {
    ...
    echo "# mountzero_v4 axon" >> "$linker_config"
...
disable_adb_root() {
    ...
    local linker_config="/linkerconfig/com.android.adbd/ld.config.txt"
    if [ -f "$linker_config" ]; then
        sed -i '/mountzero_v4 axon/d' "$linker_config" 2>/dev/null
