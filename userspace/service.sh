#!/system/bin/sh
# MountZero v4 - Service Script (Hot-plug daemon + overlay unmount watcher)
# Runs after boot-complete. Watches for new/removed modules.

MODDIR="${0%/*}"
CONFIG_DIR="/data/adb/mountzero_v4"
CONFIG_SH="$MODDIR/config.sh"
BRIDGE_SH="$MODDIR/bridge.sh"
CONFIG_FILE="$CONFIG_DIR/config.toml"
WORK_BASE="/dev/mountzero_v4_work"
UPPER_BASE="/dev/mountzero_v4_upper"
    ...
    if [ -f "$MODDIR/bin/mzctl" ]; then
        ...
        for target in /data/adb/ksu/bin/mzctl /data/adb/ap/bin/mzctl; do
            ...
            if cat "$MODDIR/bin/mzctl" > "$target" 2>/dev/null; then
                ...
                echo "mountzero_v4: mzctl installed to $target (perms: $(stat -c '%a' "$target"))" > /dev/kmsg 2>/dev/null
                break
            fi
        done
    fi

    # Install mz4 - enforce 755 permissions strictly
    if [ -f "$MODDIR/bin/susfs" ]; then
        chmod 755 "$MODDIR/bin/susfs" 2>/dev/null
        for target in /data/adb/ksu/bin/mz4 /data/adb/ap/bin/susfs; do
            mkdir -p "$(dirname "$target")" 2>/dev/null
            if cat "$MODDIR/bin/susfs" > "$target" 2>/dev/null; then
                ...
                ln -sf "$target" /data/adb/ksu/bin/mz4 2>/dev/null || true
                ln -sf "$target" /data/adb/ap/bin/ksu_mz4 2>/dev/null || true
                echo "mountzero_v4: mz4 installed to $target (perms: $(stat -c '%a' "$target"))" > /dev/kmsg 2>/dev/null
                break
            fi
        done
    fi
...
BBR_ENABLED=$(cat /data/adb/mountzero_v4/bbr_enabled 2>/dev/null)
if [ "$BBR_ENABLED" = "1" ]; then
    sysctl -w net.core.default_qdisc=fq 2>/dev/null
    sysctl -w net.ipv4.tcp_congestion_control=bbr 2>/dev/null
    echo "mountzero_v4: BBR enabled at boot" > /dev/kmsg 2>/dev/null
fi
...
# Handle ADB Root
ADB_ROOT_ENABLED=$(cat /data/adb/mountzero_v4/adb_root 2>/dev/null)
...
for moddir in /data/adb/modules/*/; do
    ...
    case "$MODID" in
        mountzero_v4_vfs|meta-mountzero_v4_vfs) continue ;;
    esac
...
unmount_module_overlays() {
    ...
    grep "mountzero_v4_${modid}_" /proc/mounts 2>/dev/null | while read -r line; do
        local mp
            ...
            if [ -d "$moddir" ] && [ ! -f "${moddir}disable" ] && [ ! -f "${moddir}remove" ]; then
                ...
                for part in system vendor product system_ext odm; do
                    if [ -d "${moddir}${part}" ]; then
                        ...
                        mount -t overlay "mountzero_v4_${modid}_${part}" \
